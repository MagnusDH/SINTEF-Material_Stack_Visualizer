...
class App:
    def __init__(self, window):
        ...
        self.material_max_thickness = 8000          #Maximum thickness of materials
        self.initial_max_thickness = self.material_max_thickness # Stores the max thickness value
    ...
    def canvas_zoom(self, event):
        ...
        if event.delta > 0:
            ...
            self.canvas.scale("all", event.x, event.y, 1.0 / self.zoom_factor, 1.0 / self.zoom_factor)

    def reset_canvas_position(self):
        #Reset the size of the canvas and all elements on it
        for item, coords in self.initial_canvas_state.items():
            self.canvas.coords(item, *coords)
        self.draw_rectangle_stack()

    def reset_values(self):
        # Reset max thickness value
        self.material_max_thickness = self.initial_max_thickness

        # Update max thickness entry widget
        self.max_thickness_entry.delete(0, tk.END)
        self.max_thickness_entry.insert(0, str(self.initial_max_thickness))
        self.max_thickness_slider.set(self.initial_max_thickness)
        self.max_thickness_slider.config(to=self.initial_max_thickness)
        
        # Update slider ranging values and reset all sliders and entry widgets
        for material, initial_value in self.initial_values.items():
            # Update slider range
            self.slider_widgets[material].config(from_=self.material_min_thickness, to=self.initial_max_thickness)

            # Reset slider
            self.slider_widgets[material].set(initial_value)

            # Reset entry box
            entry = self.entry_widgets[material]
            entry.delete(0, tk.END)
            entry.insert(0, str(initial_value))

        # Redraw the rectangle stack
        self.draw_rectangle_stack()

    def open_zoom_window(self):
        zoom_window = tk.Toplevel(self.window)
        zoom_window.title("Zoom")
        zoom_window.geometry(f"{self.window_width}x{self.window_height}")
    ...
    def create_user_interface(self):
        ...
        self.max_thickness_entry = Entry(self.slider_frame, textvariable=max_thickness_variable, justify=tk.CENTER, width=10)
        self.max_thickness_entry.grid(row=0, column=1)
        self.max_thickness_entry.bind("<Return>", lambda event, e=self.max_thickness_entry: self.update_max_thickness_entry(event, e))
        
        # Add a slider for adjusting material_max_thickness
        self.max_thickness_slider = Scale(self.slider_frame, from_=self.material_min_thickness, to=self.initial_max_thickness, orient=HORIZONTAL, resolution=1, command=self.max_thickness_slider_updated)
        self.max_thickness_slider.grid(row=0, column=2, pady=5, padx=5)
        self.max_thickness_slider.set(self.initial_max_thickness)
        ...
        for material in self.materials:
            #Create sliders
            slider = Scale(self.slider_frame, from_=self.material_min_thickness, to=self.material_max_thickness, orient=HORIZONTAL, label=material, resolution=1, command=lambda value, label=material: self.slider_updated(value, label), background=self.materials[material][1])
            slider.grid(row=row_counter, column=0, pady=5, padx=5)
            slider.set(self.materials[material][0])
            self.slider_widgets[material] = slider  # Store reference to slider



            #Create input boxes
            height_var = StringVar(value=str(self.materials[material][0]))                                          
            entry = Entry(self.slider_frame, textvariable=height_var, width=6, justify=tk.CENTER)
            entry.grid(row=row_counter, column=1, padx=5)
            entry.bind("<Return>", lambda event, s=slider, e=entry: self.entry_updated(event, e, s))
            self.entry_widgets[material] = entry  # Store reference to entry box
            ...
            # Save the initial value of each slider and entry box
            self.initial_values[material] = self.materials[material][0]
        ...
        self.reset_canvas_position_button = Button(self.slider_frame, text="Reset", command=self.reset_values)
        self.reset_canvas_position_button.grid(row=row_counter, column=2)
    ...
    def max_thickness_slider_updated(self, value):
        # Update material_max_thickness based on slider value
        self.material_max_thickness = int(value)
    ...
    def draw_rectangle_stack(self):
            self.canvas.delete("all")  # Clear all existing rectangles
            self.draw_canvas_boundaries()

            # Adjust these values to control the drawing area for rectangles
            rectangle_x0 = 2
            text_margin = 250  # Increased space for text
            rectangle_x1 = self.canvas.winfo_reqwidth() - text_margin
            rectangle_y1 = self.canvas.winfo_reqheight() - 3

            scaling_factor = self.canvas.winfo_reqheight() / ((self.material_max_thickness + 18) * len(self.materials))
            previous_text_bbox_y0 = None
            first_material_drawn = False
            i = 0

            for material in self.materials:
                rectangle_height = int(self.materials[material][0]) * scaling_factor
                rectangle_y0 = rectangle_y1 - rectangle_height
                created_rectangle = self.canvas.create_rectangle(rectangle_x0, rectangle_y0, rectangle_x1, rectangle_y1, fill=self.materials[material][1], tags="material_rectangle")

                if material == "Substrate":
                    substrate_initial_value = self.entry_widgets["Substrate"].get()  # Fetch the initial value from the entry widget
                    label_text = f"Substrate: {substrate_initial_value} nm"
                else:
                    label_text = str(material) + ': ' + str(self.materials[material][0]) + " nm"
                
                # Positioning for the text and text-boxes
                if rectangle_height >= 12:
                    label_x_pos = (rectangle_x0 + rectangle_x1) / 2
                    label_y_pos = (rectangle_y0 + rectangle_y1) / 2
                    created_text = self.canvas.create_text(label_x_pos, label_y_pos, text=label_text, fill="black", font=("Arial", self.text_size), anchor="center")
                    created_arrow_line = None
                    created_text_box = None
                else:
                    label_x_pos = self.canvas.winfo_reqwidth() - 120  # Position text to the right of the rectangle
                    label_y_pos = (rectangle_y0 + rectangle_y1) / 2
                    created_text = self.canvas.create_text(label_x_pos, label_y_pos, text=label_text, fill="black", font=("Arial", self.text_size), anchor="center")
            ...
            for item in self.canvas.find_all():
                self.initial_canvas_state[item] = self.canvas.coords(item)
    ...
    def load_materials_from_excel(self, excel_file):
        try:
            ...
            for index, row in excel_data.iterrows():
                ...
                self.materials[material_name] = [material_thickness, material_color]#, material_unit]  
            
            # Update max_thickness if the current material is not 'Substrate'
            # and its thickness is greater than the current max_thickness
                if material_name != 'Substrate' and material_thickness > max_thickness:
                    max_thickness = material_thickness
                    
                self.material_max_thickness = max_thickness
                self.initial_max_thickness = max_thickness
        ...
        for i in range(len(self.canvas_layer_data)):
            ...
            with open(file_path, 'w') as f:
                ...
                for j in range(i+1):
                    ...
                    svg_text_element = '<text x="{}" y="{}" fill="black" font-size="16" font-weight="bold" dominant-baseline="middle" text-anchor="middle">{}</text>\n'.format(label_x+25, label_y, text_content)
...
if __name__ == "__main__":
    window = tk.Tk()
    app = App(window) #Create instance of class and run application
    app.run_application()
    window.bind("<Escape>", lambda event: window.destroy()) #Closes the program if "esc" key is pressed
    window.bind("<Configure>", app.window_resized) #Checks if the program window is being resized
    window.mainloop()
    ...
    def draw_rectangle_stack_test(self):
        ...
        for material in self.materials:
            ...
            label_text = str(material) + ": " + str(self.materials[material][0]) + " nm"                                             #Text to be written
            
            
            #if the rectangle height is big enough, write material name in middle of rectangle
            if(rectangle_height >=20):
                text_x_pos = (rectangle_x0 + rectangle_x1) / 2                                                                         #X-position of label
                text_y_pos = (rectangle_y0 + rectangle_y1) / 2                                                                         #Y-position of label
                created_text = self.canvas.create_text(text_x_pos, text_y_pos, text=label_text, fill="black", font=("Arial", self.text_size), anchor="center")    #Creation of text
                created_arrow_line = None
                created_text_box = None
                ...
                created_text = self.canvas.create_text(text_x_pos, text_y_pos, text=label_text, fill="black", font=("Arial", self.text_size), anchor="center")
